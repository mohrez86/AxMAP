function [ targetAdderObject ] = CreateRandomAdderObjectWithCout( adderWidth, ...
                                 servedTiming, ...
    ...
    if tempCarryChainLength == N 
        
        FirstStage = randi( (CinBar_Co), 1, 1 );
        targetAdderObject{1} = standardLogicLibrary_CinBar_Co{FirstStage};
        tempCarryChainLength = 1;
        PreviousCarryOut = 0;
        return;

    elseif (tempCarryChainLength == K) % Then this stage cannot generate Cout
        
        FirstStage = randi( (CinBar_CoBar), 1, 1 );
        targetAdderObject{1} = standardLogicLibrary_CinBar_CoBar{FirstStage};
        tempCarryChainLength = 1;
        PreviousCarryOut = 0;
    ...
    for i = 2 : N
        ...
        if (i == N) % Must have Cout
            if (PreviousCarryOut == 0) % then choose from CinBar_Co
                IthStage = randi(CinBar_Co, 1, 1 );
                targetAdderObject{i} = standardLogicLibrary_CinBar_Co{IthStage};
            else % then choose from Cin_Co
                IthStage = randi(Cin_Co, 1, 1 );
                targetAdderObject{i} = standardLogicLibrary_Cin_Co{IthStage};
            end
            if (tempCarryChainLength > CarryChainLength)
                    CarryChainLength = tempCarryChainLength;
            end
            ...
            if (tempCarryChainLength == K) % Then this stage cannot generate Cout
                ...
                if (PreviousCarryOut == 0) % Then choose from CinBar_CoBar
                    ...
                    targetAdderObject{i} = standardLogicLibrary_CinBar_CoBar{IthStage};
                    tempCarryChainLength =  1;
                    PreviousCarryOut = 0;
                else % Then Choose from Cin_CoBar
                    IthStage = randi(Cin_CoBar, 1, 1 );
                    targetAdderObject{i} = standardLogicLibrary_Cin_CoBar{IthStage};
                    tempCarryChainLength = 1;
                    PreviousCarryOut = 0;
                end
                % Carry Chain Update
